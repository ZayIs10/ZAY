...
def _strip_vtt(vtt_text: str) -> str:
    ...
    for raw in vtt_text.splitlines():
        ...
        # Auto-captions carry HTML entities (&gt;&gt; speaker marks) and
        # non-speech tags ([music], [applause]) — noise, not spoken content.
        line = html.unescape(line)
        line = _NOISE_RE.sub("", line).strip(" >")
...
def _download_vtt(video_url: str) -> str:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # The caption fetch hits the same YouTube endpoints as the video
        # download, so on a datacenter IP (GitHub Actions) it is bot-blocked
        # the same way. Route ONLY this yt-dlp call through the residential
        # proxy — never via os.environ, which would break Sheets/Drive.
